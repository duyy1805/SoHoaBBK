...
const sxbtDefectImageUpload = multer({
  storage: multer.memoryStorage(),
...
const sxbtDefectUploadDir = path.join(__dirname, "../uploads/defects/sxbt");
if (!fs.existsSync(sxbtDefectUploadDir)) {
  fs.mkdirSync(sxbtDefectUploadDir, { recursive: true });
}
...
const slugFileName = (value) => trimValue(value)
  .normalize("NFD")
  .replace(/[\u0300-\u036f]/g, "")
  .replace(/đ/g, "d")
  .replace(/Đ/g, "D")
...
router.post(
  ...
  (req, res, next) => {
    sxbtDefectImageUpload.single("image")(req, res, (err) => {
      if (err) {
        return res.status(400).json({ message: err.message || "File ảnh không hợp lệ" });
      }
      next();
    });
  },
  async (req, res) => {
    if (!req.file) {
      return res.status(400).json({ message: "Vui lòng chọn ảnh lỗi SXBT" });
    }

    try {
      const baseName = slugFileName(req.body.maLoi || req.body.tenLoi || "loi-sxbt") || "loi-sxbt";
      const outputFilename = `sxbt-${baseName}-${Date.now()}-${Math.round(Math.random() * 1E6)}.jpg`;
      const outputPath = path.join(sxbtDefectUploadDir, outputFilename);
      ...
      res.json({
        success: true,
        imageUrl: `/uploads/defects/sxbt/${outputFilename}`
      });
    } catch (err) {
      console.error("Upload SXBT defect image error:", err);
      res.status(500).json({ message: "Không lưu được ảnh lỗi SXBT" });
    }
  }
);
